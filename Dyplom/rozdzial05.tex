...
Po zalogowaniu, użytkownik zostanie przeniesiony na główną stronę o wyglądzie jak na rysunku~\ref{fig:Logowanie}.
Na górze każdej strony będzie pasek z opcjami: Blockchain, Tokens, Cryptocurrency, Resources.
\begin{figure}[htb]
    \centering
		\begin{tabular}{ll}
    a) & \vtop{\vskip-2ex\hbox{\includegraphics[width=0.7\linewidth]{./instrukcja/Login.png}}}\\ & \\
		b) & \vtop{\vskip-2ex\hbox{\includegraphics[width=0.7\linewidth]{./instrukcja/mainpage.png}}}\\ & \\
		c) & \vtop{\vskip-2ex\hbox{\includegraphics[width=0.7\linewidth]{./instrukcja/Check_account.png}}} \\ & \\
		d) & \vtop{\vskip-2ex\hbox{\includegraphics[width=0.7\linewidth]{./instrukcja/Account_details.png}}}
		\end{tabular}
    \caption{Interfejs użytkownika: a) logowanie, b) strona główna, c) znajdź konto, d) dane konta}
    \label{fig:Logowanie}
\end{figure}
\clearpage

\subsubsection{Zakładka Blockchain}
Zakładka ta daje dostęp do opcji: Accounts, View Blocks, Transactions.

\paragraph{Konto}
Aby wyszukować konto należy wpisać adres oraz wybrać blockchain, z którego pochodzi dany adres (patrz rysunek~\ref{fig:Logowanie}c).
Po tej akcji system automatycznie przeniesie użytkownika na nową stronę z danymi konta (patrz rysunek~\ref{fig:Logowanie}d).
Sprawdzanie bloków oraz transakcji działa analogicznie.

\paragraph{Zakładka Tokens}
Zakładka ta oferuje dwie opcje: Collection oraz General NFT statistics. Jeśli użytkownik wybierze, opcję Collection, to zostanie przeniesiony na stronę z listami kolekcji zaprezentowaną na rysunku~\ref{fig:Collections}a.
Drugą opcją do wyboru jest \texttt{General NFT statistics}. Po jej wybraniu użytkownik zostanie przeniesiony na stronę jak rysunku~\ref{fig:Collections}b.
Na tej stronie użytkownik może szukać tokenów po atrybutach: Kontrakt, Identyfikator, Nazwa, Kolekcja.
\begin{figure}[htb]
    \centering
		\begin{tabular}{ll}
    a) & \vtop{\vskip-2ex\hbox{\includegraphics[width=0.8\linewidth]{./instrukcja/Collections.png}}}\\ & \\
		b) & \vtop{\vskip-2ex\hbox{\includegraphics[width=0.8\linewidth]{./instrukcja/Tokens.png}}}
		\end{tabular}
    \caption{Collections}
    \label{fig:Collections}
\end{figure}

\subsection{Zakładka Cryptocurrency}
